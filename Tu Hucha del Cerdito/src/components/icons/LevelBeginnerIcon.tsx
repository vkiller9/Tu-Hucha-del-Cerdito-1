import React from 'react';

export const LevelBeginnerIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" strokeWidth="1.5">
    <path d="M12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12C3 16.9706 7.02944 21 12 21Z" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M12 15V12M12 12V9H10.5M12 12H13.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M10 7H14" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);